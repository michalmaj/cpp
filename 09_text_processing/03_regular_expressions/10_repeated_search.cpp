/*
 * std::regex_token_iterator enables us by using indexes to specify which capture groups we are interested
 * in explicitly. If we don't specify the index, we will get all capture groups, but we can also request
 * specific capture groups using its respective index. The -1 index is special: we can use -1 to address 
